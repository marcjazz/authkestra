...
use authkestra_op::{
    ...
    config::OpConfig,
    handlers::{
        authorize::{handle_authorize, AuthorizeRequest},
...
pub async fn dummy_authorize_handler<AppState>(
    State(_state): State<AppState>,
    Query(_req): Query<std::collections::HashMap<String, String>>,
) -> Response
where
    AppState: Clone + Send + Sync + 'static,
{
...
pub async fn axum_authorize_handler<AppState>(
    ...
    Query(req_map): Query<std::collections::HashMap<String, String>>,
) -> Response
where
    AppState: Clone + Send + Sync + 'static,
    ...
    OpConfig: FromRef<AppState>,
{
    ...
    let req = AuthorizeRequest {
        client_id: req_map.get("client_id").cloned().unwrap_or_default(),
        redirect_uri: req_map.get("redirect_uri").cloned().unwrap_or_default(),
        response_type: req_map.get("response_type").cloned().unwrap_or_default(),
        scope: req_map.get("scope").cloned().unwrap_or_default(),
        state: req_map.get("state").cloned(),
        code_challenge: req_map.get("code_challenge").cloned(),
        code_challenge_method: req_map.get("code_challenge_method").cloned(),
        nonce: req_map.get("nonce").cloned(),
    };

    // TODO: Extract the actual identity from the session.
    // For now, this just passes None to simulate an unauthenticated user,
    // which should result in a redirect to the login page (or consent).
...
impl<T> AuthEngineAxumOpExt for T {
        ...
        OpConfig: FromRef<AppState>,
    {
        ...
        axum::Router::new()
            ...
            .route("/authorize", get(dummy_authorize_handler::<AppState>))
