...
JWT and token utilities for [authly-rs](https://github.com/marcjazz/authly-rs).

This crate provides JWT signing, verification, and token abstraction for use within the `authly` framework. It supports both symmetric (HS256) token management and asynchronous offline validation using JWKS.
...
- **Flexible Claims**: Standard OpenID Connect claims with support for custom fields and integrated `authly-core` Identity.
